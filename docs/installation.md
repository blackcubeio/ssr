...
`YiiSsrRoutingMiddleware`, `YiiFallbackHandler`, `YiiThrowableResponseFactory`, and `YiiLocaleManager` are **not** auto-registered. The consumer application must wire them in its own DI configuration.
...
`YiiLocaleManager` requires a `LocaleProvider` and a `SessionInterface` (both wired by yiisoft/i18n and yiisoft/session). It is auto-instantiable.
