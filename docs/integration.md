...
| Error | `YiiFallbackHandler`, `YiiThrowableResponseFactory` | Yii error handling (PSR-15) |
| Locale | `YiiLocaleManager` | Yii locale state (set/get/resolve) — session-backed |
...
`YiiSsrRoutingMiddleware`, `YiiFallbackHandler`, `YiiThrowableResponseFactory`, and `YiiLocaleManager` are wired by the consumer application (see [Installation — Yii3 setup](installation.md#yii3-setup)).
