...
| `YiiFallbackHandler` | Yii 404 handler — dispatches to CMS error handler or default |
| `YiiThrowableResponseFactory` | Yii error factory — resolves status code, dispatches to CMS error handler |
| `YiiLocaleManager` | Yii locale state — set/get/resolve, session-backed, validates against `Language::query()->active()` |
...
- [Errors](errors.md) — Yii fallback handler, throwable factory, error handler registration
- [Locale](locale.md) — YiiLocaleManager (set/get/resolve, session-backed, supported locales via Language)
