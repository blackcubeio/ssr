...
Generates `/llms.txt` **for the current locale**. Reads the `LlmMenu` tree (3 levels: root, categories, entries) and produces a [llmstxt.org](https://llmstxt.org)-conformant file:

- **Locale** — the current locale comes from the `SsrRoutingMiddlewareInterface::LOCALE_ATTRIBUTE` request attribute, set by `YiiSsrRoutingMiddleware` (see [Locale](locale.md); absent on Slim/Laravel). A **single** root is rendered: the one whose linked Content/Tag matches the locale, falling back to the first root.
- **H1** — the single `#` heading is the `PROJECT` / `NAME` parameter (read via dcore `ParameterHelper`). **Returns 404 when it is not set.**
- **Summary** — an optional `>` blockquote comes from the `PROJECT` / `SUMMARY` parameter.
- **Structure** — the root node renders as an intro link, level-2 nodes open `##` sections, level-3 nodes are list items. Links use `[name](url): description`, where `name` is the menu node name and `description` is the node description (falling back to the linked entity title).

```
# Blackcube                                ← PROJECT/NAME

> Project summary.                         ← PROJECT/SUMMARY (optional)

- [Home](https://host/en.md): Tagline.     ← root, as an intro link

## Category
- [Category](https://host/path.md): Category description.
- [Entry](https://host/path/entry.md): Entry description.
```

Entry links point to the `.md` alternate of each Content/Tag (served by `MdHandler`). Returns 404 if `PROJECT` / `NAME` is unset or no root node exists.

### `LlmFullHandler`

Generates `/llms-full.txt`. Same structure as `LlmHandler` (locale selection, `PROJECT` parameters), but each node also inlines its full markdown rendered by `ElasticMdService::renderMarkdown()`, separated from its link line by a blank line. Inlined headings are shifted by +2 levels to nest under the tree (`#` → `###`); any heading that would exceed **H6** (Markdown's deepest level) is dropped along with its content, up to the next heading.
...
Each middleware has its own `dispatch()` method. `YiiFallbackHandler` and `YiiThrowableResponseFactory` also have their own dispatch (PSR-7).
