...
Fallback handlers **never** set the locale (only valid content/tag resolutions do, via [`YiiLocaleManager`](locale.md)). The error element returned by `findByError()` uses the current locale already posted upstream.

## YiiThrowableResponseFactory
...
where `DefaultThrowableResponseFactory` is an alias for `Yiisoft\ErrorHandler\ThrowableResponseFactory`.
