...
final class YiiSsrRoutingMiddleware extends AbstractSsrRouting implements SsrRoutingMiddlewareInterface
{
    public function __construct(
        ...
        private readonly ?YiiLocaleManager $localeManager = null,
    ) {
    ...
    public function process(
        ...
        $this->localeManager?->resolve($request);
        ...
        // A valid Content/Tag (Slug-resolved) sets the current locale.
        $matches = array_filter($data, static fn($v) => $v instanceof Content || $v instanceof Tag);
        $lang = ($matches === [] ? null : reset($matches))?->getLanguageId();
        if ($lang !== null) {
            $this->localeManager?->set($lang);
        }

        // Expose the resolved locale to handlers (e.g. llms.txt) that have no path-encoded language.
        $locale = $this->localeManager?->get();
        if ($locale !== null && $locale !== '') {
            $request = $request->withAttribute(self::LOCALE_ATTRIBUTE, $locale);
        }
